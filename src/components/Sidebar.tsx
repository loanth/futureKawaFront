...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  ...
  const [currentCountryConfig, setCurrentCountryConfig] = useState(
    multiCountryApiService.getCurrentCountryConfig()
  );

  // ✅ La supervision est déterminée par idPoste === 3
  ...
  const handleCountryClick = (countryCode: string) => {
    if (isSupervisor) {
      ...
      localStorage.setItem(
        'countryConfig',
        JSON.stringify(COUNTRIES_CONFIG.find(c => c.code === countryCode))
      );
  ...
  const navItems: NavItem[] = [
    // Dashboard uniquement visible pour les superviseurs (idPoste === 3)
    ...(isSupervisor
      ? [{ path: '/', label: t('navigation.dashboard'), icon: Home }]
      : []),

    // Pays : tous les pays pour superviseur, pays actuel sinon
    ...(isSupervisor
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 border-b border-coffee-medium">
          ...
          <button className="lg:hidden text-cream-bg" onClick={() => setIsOpen(false)}>
            <X size={24} />
          </button>
        </div>
                ...
                <NavLink
                  ...
                  className={({ isActive }) => `
                    ...
                    ${isActive
                      ...
                      : 'hover:bg-coffee-medium text-cream-bg/80 hover:text-white'}
                  `}
                >
                  <div className="flex items-center space-x-3">
                    <item.icon size={20} />
        ...
        <div className="mt-auto p-6 border-t border-coffee-medium">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-coffee-light truncate w-32">{user?.mail}</p>
            </div>
          </div>
          <button
